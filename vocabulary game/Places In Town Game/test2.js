...
function emitParticles(x,y,count=25,colors=['#818cf8','#f472b6','#34d399','#fbbf24','#38bdf8']) { for(var i=0;i<count;i++) particles.push(new Particle(x,y,colors[Math.floor(Math.random()*colors.length)])); }
function animateParticles() { ctx.clearRect(0,0,canvas.width,canvas.height); particles=particles.filter(function(p)p.life>0); particles.forEach(function(p){p.update();p.draw()}); requestAnimationFrame(animateParticles); }
animateParticles();
...
function playTone(freq,dur,type='sine',vol=0.15) { try { var c=getAudioCtx(),o=c.createOscillator(),g=c.createGain();o.type=type;o.frequency.value=freq;g.gain.setValueAtTime(vol,c.currentTime);g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+dur);o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+dur); } catch(e){} }
function sfxCorrect(){playTone(523,0.1);setTimeout(function()playTone(659,0.1),100);setTimeout(function()playTone(784,0.15),200)}
function sfxIncorrect(){playTone(200,0.2,'sawtooth',0.08)}
function sfxFlip(){playTone(440,0.05,'sine',0.08)}
function sfxWin(){[523,659,784,1047].forEach((f,i)=>setTimeout(function()playTone(f,0.15),i*120))}
...
function speakText(text) {
    ...
    var en=voices.find(function(v)v.lang.startsWith('en')&&v.name.includes('Emma'))||voices.find(function(v)v.lang.startsWith('en-'));
    if(en) u.voice=en;
...
function addXP(amount) {
    gameState.xp+=amount; var xpPer=100;
...
// ====================== TOAST ======================
function showToast(msg,type='info'){var t=document.createElement('div');t.className=`toast ${type}`;t.textContent=msg;document.body.appendChild(t);setTimeout(function()t.remove(),2500)}

// ====================== NAVIGATION ======================
function showScreen(id){document.querySelectorAll('.screen').forEach(function(s)s.classList.remove('active'));document.getElementById(id).classList.add('active');window.scrollTo({top:0,behavior:'smooth'})}
...
function initVocabPanel() {
    ...
    var cats = ['noun','verb','adj','adv','prep','det','pron','conj','phrase'];
    var labels = {noun:'Danh từ', verb:'Động từ', adj:'Tính từ', adv:'Trạng từ', prep:'Giới từ', det:'Hạn định', pron:'Đại từ', conj:'Liên từ', phrase:'Cụm từ'};
    pills.innerHTML = cats.map(function(c) {
        var count = c === 'phrase' ? 100 : allVocab.filter(function(v)v.cat===c).length;
...
function filterVocab(cat) {
    currentFilter = cat;
    document.querySelectorAll('#filterPills .filter-pill').forEach(function(p)p.classList.toggle('active',p.dataset.cat===cat));
    renderVocabList();
}
function renderVocabList() {
    ...
    if (currentFilter === 'phrase') {
        var commPhrases = phrases.slice(-100);
        items = commPhrases.map(function(p) {
            var parsed = parseTemplate(p.template);
            var engStr = parsed.map(function(s) s.type==='text' ? s.value : s.answer).join('');
            return {
                word: engStr.trim(),
                cat: 'phrase',
                ipa: 'phrase',
        ...
        items = allVocab.filter(function(v)v.cat===currentFilter);
    }
...
// ====================== 1. WORD EXPLORER ======================
var fcList = [], fcIndex = 0, fcFilter = 'noun';
function initExplorer() {
    fcFilter = 'noun'; fcIndex = 0;
    buildFcFilters(); buildFcList(); updateFlashcard();
}
function buildFcFilters() {
    var el = document.getElementById('fcFilterPills');
    if (!el) return;
    var cats = ['noun','verb','adj','adv','prep','det','pron','conj','phrase'];
    var labels = {noun:'n.',verb:'v.',adj:'adj.',adv:'adv.',prep:'prep.',det:'det.',pron:'pron.',conj:'conj.',phrase:'cụm từ'};
    el.innerHTML = cats.map(function(c)`<button class="filter-pill ${c===fcFilter?'active':''}" data-cat="${c}" onclick="fcFilterCat('${c}')">${labels[c]}</button>`).join('');
}
function fcFilterCat(cat){fcFilter=cat;fcIndex=0;buildFcList();updateFlashcard();document.querySelectorAll('#fcFilterPills .filter-pill').forEach(function(p)p.classList.toggle('active',p.dataset.cat===cat))}
function buildFcList(){
    if (fcFilter === 'phrase') {
        var commPhrases = phrases.slice(-100);
        fcList = commPhrases.map(function(p) {
            var parsed = parseTemplate(p.template);
            var engStr = parsed.map(function(s) s.type==='text' ? s.value : s.answer).join('');
            return {
                word: engStr.trim(),
        ...
        fcList=allVocab.filter(function(v)v.cat===fcFilter).sort(function() Math.random() - 0.5);
    }
}
function updateFlashcard(){
    if(!fcList.length) return;
    var v=fcList[fcIndex];
...
// ====================== 2. QUIZ CHALLENGE ======================
var quizQs=[],quizIdx=0,quizScoreVal=0,quizCorrectCount=0,quizTimerInterval=null,quizTimeLeft=15;
...
function generateQuiz(){
    var qs=[];
    var quizVocab = allVocab.filter(function(v)!['det','pron','conj'].includes(v.cat));
    var sh=[...quizVocab].sort(function()Math.random()-0.5);
    for(var i=0;i<Math.min(50,sh.length);i++){
        var v=sh[i]; var r=Math.random();
        if(r<0.4){
            // en -> vi
            var others=quizVocab.filter(function(x)x.word!==v.word).sort(function()Math.random()-0.5).slice(0,3);
            qs.push({question:`"${v.word}" nghĩa là gì?`,icon:v.icon,context:`(${CAT[v.cat].label})`,options:[{text:v.vi,correct:true},...others.map(function(o)({text:o.vi,correct:false}))].sort(function()Math.random()-0.5)});
        } else if(r<0.7){
            // vi -> en
            var others=quizVocab.filter(function(x)x.word!==v.word).sort(function()Math.random()-0.5).slice(0,3);
            qs.push({question:`"${v.vi}" trong tiếng Anh là gì?`,icon:v.icon,context:'',options:[{text:v.word,correct:true},...others.map(function(o)({text:o.word,correct:false}))].sort(function()Math.random()-0.5)});
        } else {
            // word type identification
            var otherCats=Object.keys(CAT).filter(function(c)!['det','pron','conj'].includes(c) && c!==v.cat).sort(function()Math.random()-0.5).slice(0,3);
            qs.push({question:`"${v.word}" thuộc loại từ nào?`,icon:v.icon,context:`(${v.vi})`,options:[{text:CAT[v.cat].label,correct:true},...otherCats.map(function(c)({text:CAT[c].label,correct:false}))].sort(function()Math.random()-0.5)});
        }
    }
    return qs;
}
function loadQuizQ(){
    if(quizIdx>=quizQs.length){if(quizTimerInterval)clearInterval(quizTimerInterval);var acc=Math.round((quizCorrectCount/quizQs.length)*100);showResult(acc>=80?'🏆':acc>=50?'👏':'💪',acc>=80?'Quiz Master!':acc>=50?'Good Try!':'Keep Learning!',`Đúng ${quizCorrectCount}/${quizQs.length} câu`,quizScoreVal,acc);return}
    ...
    quizTimerInterval=setInterval(function(){quizTimeLeft-=0.1;document.getElementById('quizTimerFill').style.width=(quizTimeLeft/15*100)+'%';document.getElementById('quizTimer').textContent=Math.ceil(quizTimeLeft);if(quizTimeLeft<=0){clearInterval(quizTimerInterval);sfxIncorrect();showToast('⏰ Hết giờ!','error');highlightQuiz();setTimeout(function(){quizIdx++;loadQuizQ()},1500)}},100);
}
function highlightQuiz(){var q=quizQs[quizIdx];document.querySelectorAll('.quiz-option').forEach((b,i)=>{b.style.pointerEvents='none';if(q.options[i].correct)b.classList.add('correct')})}
function answerQuiz(idx,ok){
    ...
    if(ok){btns[idx].classList.add('correct');quizScoreVal+=10+Math.round(quizTimeLeft);quizCorrectCount++;document.getElementById('quizScore').textContent=quizScoreVal;sfxCorrect();var r=btns[idx].getBoundingClientRect();emitParticles(r.left+r.width/2,r.top+r.height/2,20)}
    else{btns[idx].classList.add('incorrect');highlightQuiz();sfxIncorrect()}
    setTimeout(function(){quizIdx++;loadQuizQ()},1200);
}

// ====================== 3. PHRASE FORGE ======================
var forgeList=[],forgeIdx=0,forgeScoreVal=0,forgeCorrectCount=0,forgeParsed=[],forgeAnswers=[];
function initForge(){forgeList=[...phrases];forgeIdx=0;forgeScoreVal=0;forgeCorrectCount=0;document.getElementById('forgeScore').textContent='0';
    var sel = document.getElementById('forgeSelector');
    if(sel) {
        sel.innerHTML = forgeList.map(function(p, i) `<option value="${i}">${i+1}. ${p.vi}</option>`).join('');
    }
    loadForge();
}
...
function loadForge(){
    ...
    if(forgeIdx>=forgeList.length){var acc=Math.round((forgeCorrectCount/forgeList.length)*100);showResult(acc>=70?'🏆':'💪',acc>=70?'Phrase Master!':'Nice Try!',`Đúng ${forgeCorrectCount}/${forgeList.length} cụm từ`,forgeScoreVal,acc);return}
    var p=forgeList[forgeIdx];
    forgeParsed=parseTemplate(p.template);
    forgeAnswers=new Array(forgeParsed.filter(function(s)s.type==='blank').length).fill(null);
...
function renderForgeSlots(){
    var slotsEl=document.getElementById('forgeSlots');
    var blankIdx=0;
    slotsEl.innerHTML=forgeParsed.map(function(seg){
        if(seg.type==='text') return `<span style="font-size:1.1rem;color:var(--text-muted)">${seg.value}</span>`;
        var i=blankIdx++;
        var c=CAT[seg.cat];
        var filled=forgeAnswers[i];
...
function renderForgeBanks(){
    var banksEl=document.getElementById('forgeBanks');
    var neededCats=[...new Set(forgeParsed.filter(function(s)s.type==='blank').map(function(s)s.cat))];
    banksEl.innerHTML=neededCats.map(cat=>{
        var c=CAT[cat];
        var correctWords=forgeParsed.filter(function(s)s.type==='blank'&&s.cat===cat).map(function(s)s.answer);
        var catWords=allVocab.filter(function(v)v.cat===cat).map(function(v)v.word);
        
        var uniqueCorrect = [...new Set(correctWords)];
        var distractors = [...new Set(catWords)].filter(function(w) !uniqueCorrect.map(function(u)u.toLowerCase()).includes(w.toLowerCase())).sort(function()Math.random()-0.5);
        var finalWords = [...uniqueCorrect, ...distractors.slice(0, Math.max(0, 5 - uniqueCorrect.length))].sort(function()Math.random()-0.5);

        var usedWords=forgeAnswers.filter(function(a)a!==null).map(function(a)a.toLowerCase());
        return `<div class="forge-bank"><div class="forge-bank-label"><span class="cat-tag cat-${cat}">${c.short}</span> ${c.label}</div>
            <div class="forge-bank-chips">${finalWords.map(function(w)`<button class="forge-chip ${usedWords.includes(w.toLowerCase())?'used':''}" style="background:${c.bg};color:${c.color};border-color:${c.border}" onclick="forgePick('${w.replace(/'/g,"\\'")}','${cat}')">${w}</button>`).join('')}</div></div>`;
    }).join('');
}
function forgePick(word,cat){
    var blanks=forgeParsed.filter(function(s)s.type==='blank');
    var targetIdx=-1;
    for(var i=0;i<blanks.length;i++){if(forgeAnswers[i]===null&&blanks[i].cat===cat){targetIdx=i;break;}}
    if(targetIdx===-1){for(var i=0;i<blanks.length;i++){if(forgeAnswers[i]===null){targetIdx=i;break;}}}
    if(targetIdx===-1) return;
    forgeAnswers[targetIdx]=word;
    sfxClick();
    renderForgeSlots();renderForgeBanks();
    if(forgeAnswers.every(function(a)a!==null)) setTimeout(function()forgeCheck(),400);
}
...
function forgeClear(){forgeAnswers=forgeAnswers.map(function()null);sfxClick();renderForgeSlots();renderForgeBanks()}
function jumpForge(idx){ forgeIdx = parseInt(idx,10); loadForge(); }
function jumpSf(idx){ sfIdx = parseInt(idx,10); loadSentenceForge(); }
function forgeCheck(){
    var blanks=forgeParsed.filter(function(s)s.type==='blank');
    if(forgeAnswers.some(function(a)a===null)){showToast('Hãy điền đủ tất cả ô!','info');return}
    var allOk=true;
    var slots=document.querySelectorAll('.forge-slot[data-idx]');
    blanks.forEach((b,i)=>{
        var ok=forgeAnswers[i].toLowerCase()===b.answer.toLowerCase();
        if(!ok) allOk=false;
    ...
    if(allOk){forgeScoreVal+=10;forgeCorrectCount++;document.getElementById('forgeScore').textContent=forgeScoreVal;sfxCorrect();showToast('🎉 Chính xác!','success');var el=document.getElementById('forgeSlots');var r=el.getBoundingClientRect();emitParticles(r.left+r.width/2,r.top+r.height/2);forgeIdx++;setTimeout(loadForge,1200)}
    else{
        sfxIncorrect();
...
var sfIdx = 0;
var sfScoreVal = 0;
var sfCorrectCount = 0;
var sfCurrentPhrase = null;
var sfParsed = [];
var sfAnswers = []; 
var sfPool = []; 
var sfPoolUsed = []; 
...
function initSentenceForge() {
    sfList = phrases.filter(function(p) p.template.includes('{{pron|') || p.template.includes('{{verb|is}}') || p.template.includes('{{verb|crosses}}') || p.template.includes('{{verb|depends}}'));
    sfIdx = 0; 
    sfScoreVal = 0; 
    sfCorrectCount = 0;
    ...
    if(sel) {
        sel.innerHTML = sfList.map(function(p, i) `<option value="${i}">${i+1}. ${p.vi}</option>`).join('');
    }
    loadSentenceForge();
}
...
function loadSentenceForge() {
    ...
    // Extract words
    var targetWords = sfParsed.map(function(s) s.type==='text' ? s.value.trim() : s.answer.trim()).filter(function(w) w.length > 0);
    
    // Add 1-2 distractors from allVocab (random)
    var distractors = [...allVocab].sort(function()Math.random()-0.5).slice(0, 2).map(function(v) v.word);
    
    sfPool = [...targetWords, ...distractors].sort(function()Math.random()-0.5);
    sfPoolUsed = sfPool.map(function() false);
    sfAnswers = [];
...
function renderSf() {
    var slotsArea = document.getElementById('sfSlotsArea');
    var bankArea = document.getElementById('sfBankArea');
...
function sfRemove(ansIdx) {
    var word = sfAnswers[ansIdx];
    sfAnswers.splice(ansIdx, 1);
    for(var i=0; i<sfPool.length; i++) {
...
function sfClear() {
    sfPoolUsed = sfPoolUsed.map(function()false);
    sfAnswers = [];
    sfxClick();
    renderSf();
}
...
function sfCheck() {
    ...
    var answerStr = sfAnswers.join(' ').toLowerCase();
    var targetStr = targetWords.join(' ').toLowerCase();
    ...
    if(answerStr === targetStr) {
        ...
        var el = document.getElementById('sfSlotsArea');
...
var isAutoSpeakerOn = false;
var currentAutoSentenceText = '';

function toggleAutoSpeaker() {
    isAutoSpeakerOn = !isAutoSpeakerOn;
    var btn = document.getElementById('autoSpeakerBtn');
    if (isAutoSpeakerOn) {
...
function runAutoSentence() {
    var banner = document.getElementById('autoSentenceBanner');
    if(!banner) return;
    var commPhrases = phrases.slice(-100);
    var p = commPhrases[Math.floor(Math.random() * commPhrases.length)];
    var parsed = parseTemplate(p.template);
    
    currentAutoSentenceText = parsed.map(function(s) s.type==='text' ? s.value : s.answer).join('');
...
initVocabPanel();
runAutoSentence();
if(window.speechSynthesis){window.speechSynthesis.getVoices();if(window.speechSynthesis.onvoiceschanged!==undefined)window.speechSynthesis.onvoiceschanged=function()window.speechSynthesis.getVoices()}
